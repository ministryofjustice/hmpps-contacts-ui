...
export default class ContactsSearchController implements PageHandler {
  constructor(private readonly contactsService: ContactsService) {}

  public PAGE_NAME = Page.DIRECT_CONTACT_SEARCH_PAGE
  ...
  private validateRequest(journey: SearchContactJourney, res: e.Response) {
  ...
  POST = async (req: Request<{ journeyId: string }>, res: Response): Promise<void> => {
    ...
    // Use any to avoid TS session typing issues if session typings aren't present.
    const { journeyId } = req.params
    const existingJourney = req.session.searchContactJourneys![journeyId]!
