...
describe(`GET /prisoner/:prisonerNumber/contacts/create/addresses/:addressIdx/enter-address/:journeyId`, () => {
  it('should render enter address page', async () => {
    // When
    const response = await request(app).get(
    ...
    const addressLines = existingJourney.addressesToSave![0]!.addressLines!
...
describe('POST /prisoner/:prisonerNumber/contacts/create/addresses/:addressIdx/enter-address/:journeyId', () => {
  ...
  it('should update journey data and bounce back', async () => {
    ...
    expect(session.addContactJourneys![journeyId]!.addressesToSave![0]!.addressLines!).toStrictEqual({
      noFixedAddress: false,
      countryCode: 'ENG',
    })
  })
