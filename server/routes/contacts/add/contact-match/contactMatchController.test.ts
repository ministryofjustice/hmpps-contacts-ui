...
describe('Contact details', () => {
  ...
  describe('GET /prisoner/:prisonerNumber/contacts/EXISTING/match/:journeyId?contactId=', () => {
    ...
    it('GET should block access without edit contacts permission', async () => {
      ...
      await request(app).get(`/prisoner/${prisonerNumber}/contacts/add/match/22/${journeyId}`).expect(403)
    })
  })
  ...
  describe('Linked prisoners tab', () => {
    ...
    it('should load the relevant page if not default', async () => {
      ...
      const response = await request(app).get(
        `/prisoner/${prisonerNumber}/contacts/add/match/22/${journeyId}?linkedPrisonerPage=2`,
      )
      const $ = cheerio.load(response.text)
  ...
  describe('Restrictions', () => {
    ...
    describe('Global Restrictions', () => {
      describe('GET /prisoner/:prisonerNumber/contacts/add/match/:matchingContactId/:journeyId?#restrictions', () => {
...
describe('POST /prisoner/:prisonerNumber/contacts/add/match/22/:journeyId?contactId=', () => {
  it('should pass validation when "Yes" is selected, set the contact id and mode to EXISTING', async () => {
    ...
    await request(app)
      ...
      .type('form')
      .send({ isContactMatched: 'YES' })
      .expect(302)
      .expect('Location', `/prisoner/${prisonerNumber}/contacts/add/mode/EXISTING/${journeyId}`)

    // Then
    expect(existingJourney.isContactMatched).toStrictEqual('YES')
    expect(existingJourney.mode).toStrictEqual('EXISTING')
    expect(existingJourney.contactId).toStrictEqual(22)
  })

  it('should pass validation when "No, take me back to contact search to try again" is selected and return to search', async () => {
    // Given
    existingJourney.matchingContactId = 22
    ...
    await request(app)
      .post(`/prisoner/${prisonerNumber}/contacts/add/match/22/${journeyId}`)
      .type('form')
      .send({ isContactMatched: 'NO_SEARCH_AGAIN' })
      .expect(302)
      .expect('Location', `/prisoner/${prisonerNumber}/contacts/search/${journeyId}`)

    // Then
    expect(session.addContactJourneys![journeyId]!.isContactMatched).toStrictEqual('NO_SEARCH_AGAIN')
  })

  it('should pass validation when "No, I need to add a new contact onto the system" is selected and set mode to NEW', async () => {
    // Given
    existingJourney.matchingContactId = 22
    ...
    await request(app)
      ...
      .expect(302)
      .expect('Location', `/prisoner/${prisonerNumber}/contacts/add/mode/NEW/${journeyId}`)
