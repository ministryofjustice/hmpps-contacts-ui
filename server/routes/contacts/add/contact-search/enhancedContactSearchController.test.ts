...
describe('GET /prisoner/:prisonerNumber/contacts/enhanced-search/:journeyId', () => {
  it('should render contact page without filter when there is no search', async () => {
    ...
    // When
    const response = await request(app).get(`/prisoner/${prisonerNumber}/contacts/enhanced-search/${journeyId}`)
    ...
    expect(auditService.logPageView).toHaveBeenCalledWith(Page.ENHANCED_CONTACT_SEARCH_PAGE, {
  ...
  it('GET should block access without edit contacts permission', async () => {
    ...
    await request(app).get(`/prisoner/${prisonerNumber}/contacts/enhanced-search/${journeyId}`).expect(403)
  })
})

describe('POST /prisoner/:prisonerNumber/contacts/enhanced-search/:journeyId', () => {
  it('should pass to result page when last name provided', async () => {
    await request(app)
      .post(`/prisoner/${prisonerNumber}/contacts/enhanced-search/${journeyId}`)
      .type('form')
      .send({ lastName: 'last', middleNames: '', firstName: '' })
      .expect(302)
      .expect('Location', `/prisoner/${prisonerNumber}/contacts/enhanced-search/${journeyId}#`)
