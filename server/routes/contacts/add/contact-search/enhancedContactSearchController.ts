...
export default class EnhancedContactSearchController implements PageHandler {
  constructor(private readonly contactsService: ContactsService) {}

  public PAGE_NAME = Page.ENHANCED_CONTACT_SEARCH_PAGE
  ...
  GET = async (
    ...
    if (journey.searchContact) {
      ...
      if (clear === 'filter') {
        ...
        return res.redirect(`/prisoner/${journey.prisonerNumber}/contacts/enhanced-search/${journeyId}#`)
      }
      if (sort) {
        journey.searchContact.sort = sort
        journey.searchContact.page = 1
        return res.redirect(`/prisoner/${journey.prisonerNumber}/contacts/enhanced-search/${journeyId}#pagination`)
      }
      if (page) {
    ...
    return res.render('pages/contacts/manage/enhancedContactSearch', view)
  }
  ...
  POST = async (req: Request<{ journeyId: string }, ContactSearchSchemaType>, res: Response): Promise<void> => {
    ...
    res.redirect(`/prisoner/${journey.prisonerNumber}/contacts/enhanced-search/${journeyId}#`)
  }
