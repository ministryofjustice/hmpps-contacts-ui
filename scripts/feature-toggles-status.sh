...
FEATURE_SEARCH_BY_CONTACT_ID_ENABLED=$(kubectl -n "$CONTACTS_UI_NAMESPACE" get secret feature-toggles -o jsonpath='{.data.FEATURE_SEARCH_BY_CONTACT_ID_ENABLED}' | base64 -d)
...
echo Contacts ui - FEATURE_SEARCH_BY_CONTACT_ID_ENABLED="$FEATURE_SEARCH_BY_CONTACT_ID_ENABLED"
